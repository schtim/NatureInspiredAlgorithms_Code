...
# nur die erste Idee für die Grid search -> wurde später durch das Notebook Grid Search Vis ersetzt -> einfacher auszuwerten
                ...
                for number_gens in number_generation_vals:
                    for fitness_function in fitness_function_vals:
                        ...
                        for index in np.arange(av_number):
                            GA = GeneticAlgorithm(small_objects, small_container[0], small_container[1], number_gens, pop_size, crossover_prob, mutation_prob, fitness_function, 'first_fit')
                            _,_,all_time_best[index], average_vals[index], _,_,runtime[index],_ = GA.run()
                        # average the vals
            ...
            for mutation_prob in mutation_probability_vals:
                for number_gens in number_generation_vals:
                    for fitness_function in fitness_function_vals:
                        ...
                        runtime = np.zeros(av_number)
                        for index in np.arange(av_number):
                            GA = GeneticAlgorithm(medium_objects, medium_container[0], medium_container[1], number_gens, pop_size, crossover_prob, mutation_prob, fitness_function, 'first_fit')
                            _,_,all_time_best[index], average_vals[index], _,_,runtime[index],_ = GA.run()
                        # average the vals
                        average_all_time_best = np.average(all_time_best)
                        global_all_time_best = np.min(all_time_best)
            ...
            for mutation_prob in mutation_probability_vals:
                for number_gens in number_generation_vals:
                    for fitness_function in fitness_function_vals:
                        ...
                        runtime = np.zeros(av_number)
                        for index in np.arange(av_number):
                            GA = GeneticAlgorithm(large_objects, large_container[0], large_container[1], number_gens, pop_size, crossover_prob, mutation_prob, fitness_function, 'first_fit')
                            _,_,all_time_best[index], average_vals[index], _,_,runtime[index],_ = GA.run()
                        # average the vals
                        average_all_time_best = np.average(all_time_best)
                        global_all_time_best = np.min(all_time_best)
